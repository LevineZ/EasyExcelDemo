...
import com.easyexcel.data.write.DemoData;
import com.easyexcel.data.write.IndexData;
import com.easyexcel.util.TestFileUtil;
import com.easyexcel.data.write.*;
...
// @SpringBootTest(classes = { ExcelAppilication.class }) // 指定启动类
public class WriteTest {
	...
	public void repeatedWrite() {
		...
		// 去调用写入,这里我调用了五次，实际使用时根据数据库分页的总的页数来。这里最终会写到5个sheet里面
		for (int i = 0; i < 5; i++) {
			// 每次都要创建writeSheet 这里注意必须指定sheetNo。这里注意DemoData.class
	...
	public void longestMatchColumnWidthWrite() {
		String fileName = TestFileUtil.getPath() + "longestMatchColumnWidthWrite" + System.currentTimeMillis()
				+ ".xlsx";
		// 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
	 ...
	 * <p>
	 * demo这里实现2点。1. 对第一行第一列的头超链接到:https://github.com/alibaba/easyexcel 2.
